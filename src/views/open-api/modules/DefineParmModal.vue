<template>
  ...
  >
    <a-form :form="form">
      ...
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="是否必填">
        <a-input placeholder="是否必填" v-decorator="['isRequired']"/>
      </a-form-item>
      <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="默认值">
...
<script>
import { saveDefineParm } from '@/api/open-api/defineParm'
import pick from 'lodash.pick'
export default {
  name: 'DefineParmModal',
  props: {
  },
  components: {
  },
  ...
  methods: {
    ...
    edit (record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.mdl, 'id','apiId','zhName','enName','type','isRequired','defaultValue','createTime','updateTime','remarks','delFlag'))
      })
    },
    handleSubmit (e) {
      e.preventDefault()
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          saveDefineParm(values).then(res => {
            if (res.code === 0) {
